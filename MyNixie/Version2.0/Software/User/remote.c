...
//	NVIC_InitTypeDef NVIC_InitStructure;	//中断
	...
	GPIO_EXTILineConfig(GPIO_PortSourceGPIOB, GPIO_PinSource7);  	//选择PA1所在的GPIO管脚用作外部中断线路EXIT1		
...
u8 Pulse_Width_Check(void)
{
    ...
    {	 
      t++;
      //例程精确定时为20us很准，但是对于粗略延时来说仿真为150us才好用
...
{       
  ...
  {  
    ...
    EXTI_ClearITPendingBit(EXTI_Line7);  //清除EXTI1线路挂起位   
  }    
}  
//处理红外键盘
//返回相应的键值
